...
def login():
    if user_name.get() == "" or password.get() == "":
        ...
        try:
            con = mysql.connector.connect(host="localhost", user="root", password="", database="placement")
            cur = con.cursor()

            cur.execute("select * from students where Full_name=%s and Password=%s", (user_name.get(), password.get()))
            row = cur.fetchall()
