...
def DisplayData3():
    mydb = mysql.connector.connect(host='localhost', user='root', password='', database='placement')
    cur = mydb.cursor()
...
def update3():
    ...
    cur = mydb.cursor()
    cur.execute(
        "update selected set Full_name = %s,Company_name=%s,CGPA=%s,Sector=%s,Year_of_joining=%s where Reg_no =%s ",
...
def delete3():
    ...
    mydb = mysql.connector.connect(host='localhost', user='root', password='', database='placement')
    cur = mydb.cursor()
    sql = 'delete from selected where Reg_no="%s"' % reg3_var.get()
    cur.execute(sql)
    mydb.commit()
...
def search3():
    ...
    mydb = mysql.connector.connect(host='localhost', user='root', password='', database='placement')
    cur = mydb.cursor()
    sql = 'select * from selected where Reg_no="%s"' % search3_var.get()
    cur.execute(sql)
    result = cur.fetchone()
...
# sub_b3 = Button(text='Submit',padx=5,pady=5,command=submit3)
# sub_b3.place(x=45,y=550)
#
# can_b3= Button(text='Cancel',padx=5,pady=5,command=cancel3)
# can_b3.place(x=110,y=550)
#
# upd_b3 = Button(text='Update',padx=5,pady=5,command=update3)
# upd_b3.place(x=175,y=550)
#
# dlt_b3 = Button(text='Delete',padx=5,pady=5,command=delete3)
# dlt_b3.place(x=240,y=550)
#
# reset_b3 = Button(text='Reset',padx=5,pady=5,command=reset3)
# reset_b3.place(x=305,y=550)
