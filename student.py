...
f2 = Frame(window, width=350, height=500, bg="#37948e")
f2.place(x=30, y=140)
...
def submit():
    ...
    mydb = mysql.connector.connect(host='localhost', user='root', password='', database='placement')
    cur = mydb.cursor()
    cur.execute("insert into student_table values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
                (name_var.get(), email_var.get(), phone_var.get(), gender_var.get(), reg_var.get(), bran_var.get(),
                 year_var.get(), cgpa_var.get(), hsc_var.get(), ssc_var.get()))
    cur.execute("Select * from student_table")
    rows = cur.fetchall()
    for row in rows:
        student_table.insert('', END, values=row)
...
def update():
    ...
    cur = mydb.cursor()
    cur.execute(
        "update student_table set Full_name = %s,Email_Id=%s,Phone_no=%s,Gender=%s,Branch=%s,Year=%s,CGPA=%s,HSC=%s,SSC=%s where Registration_no =%s ",
        (name_var.get(), email_var.get(), phone_var.get(), gender_var.get(), bran_var.get(), year_var.get(),
         cgpa_var.get(), hsc_var.get(), ssc_var.get(), reg_var.get()))
    cur.execute("Select * from student_table")
    rows = cur.fetchall()
    for row in rows:
        student_table.insert('', END, values=row)
...
def search():
    ...
    mydb = mysql.connector.connect(host='localhost', user='root', password='', database='placement')
    cur = mydb.cursor()
    sql = 'select * from student_table where Registration_no="%s"' % search_var.get()
    cur.execute(sql)
    result = cur.fetchone()
...
def delete():
    ...
    mydb = mysql.connector.connect(host='localhost', user='root', password='', database='placement')
    cur = mydb.cursor()
    sql = 'delete from student_table where Registration_no="%s"' % reg_var.get()
    cur.execute(sql)
    mydb.commit()
